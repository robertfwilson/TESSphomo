...
class TESSTargetPixelModeler(object):
    ...
    def plot_tpf_model(self, plot_color='C1', logscale=True, vmin=None, vmax=None, plot_bkg_stars=False,**kwargs):
        ...
        for ax in (ax1,ax2,ax3):
            ...
            if plot_bkg_stars:
                ax.scatter(star_rowcol.T[0]-(self.row_ref), star_rowcol.T[1]-self.col_ref, s=(star_mags- TESS_ZEROPOINT_MAG)**2., marker='*', edgecolor='0.5', color='w' , )
                
            ax.scatter(star_rowcol.T[0][0]-(self.row_ref), star_rowcol.T[1][0]-self.col_ref, s=(star_mags[0]- TESS_ZEROPOINT_MAG)**2., marker='*', edgecolor=plot_color, color='w' , zorder=5)
    ...
    def get_corrected_LightCurve(self, err_exponent=1.0, aperture=None, bad_data_mask=None, assume_catalog_mag=False,
                                 ...
                                 do_pca=False, model_straps=True, bkg_poly_order=1, decorrelate_offset=True, **kwargs):
        ...
        if decorrelate_offset:
            all_systematics = np.concatenate([np.array(systematics).T, [dx_t, dy_t, dx_t*dy_t]])
        else:
            all_systematics = np.array(systematics).T
